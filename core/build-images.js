 ...
 * @property {import('canvas').NodeCanvasRenderingContext2DSettings} ctx
 ...
 */
const BuildImages = (comments) =>
  Promise.all(
    comments.filter(Boolean).map((commentData) => {
       ...
       * @returns {import('../types').GotLinesType}
       */
      const GetLines = (text, maxTextWidth, fontSizeInLine, fontWeight = '400', additionalLinesParams = {}) => {
        ...
        /** @type {import('../types').ComplexMiddleLine[]} */
        ...
        linesSplittedByNewLineChar.forEach((lineSplittedByNewLineChar) => {
          ...
          while (lineToCount.length) {
            for (
              i = lineToCount.length;
              ctxForTest.measureText(lineToCount.substr(0, i)).width >
              (additionalLinesParams?.verified ? maxTextWidth - fontSizeInLine * 2 : maxTextWidth);
              i--
            );

            result = lineToCount.substr(0, i);

            if (i !== lineToCount.length) for (j = 0; result.indexOf(' ', j) !== -1; j = result.indexOf(' ', j) + 1);
          ...
          localLines.forEach((localLine, localLineIndex) => {
            /** @type {import('../types').AdditionalTextEntity[]} */
      ...
      /** @type {import('../types').GotLinesType} */
      ...
      /** @type {(import('../types').RemoteImageToDraw | import('../types').LocalImageToDraw)[]} */
      const imagesToDraw = [];
      ...
      if (commentData.attaches?.length) {
        ...
        commentData.attaches
          ...
          .forEach(
            /** @param {import('../types').CommentAttachTypeMedia} media */ (media) => {
              const ratio = media.data.width / media.data.height;

              let widthForImage = media.data.width > 1800 ? 1800 : media.data.width < 1500 ? 1500 : media.data.width;
      ...
      let karmaTextColor = '#555555';
      let karmaText = commentData.likes?.count?.toString() || '0';

      if (commentData.likes?.count < 0) {
        karmaBackgroundColor = '#FFF1F1';
        karmaTextColor = '#CD192E';
      } else if (commentData.likes?.count > 0) {
        karmaText = `+${karmaText}`;
        karmaBackgroundColor = '#DDF6E7';
      ...
      const karmaWidth = karmaMetrics.width;
      const karmaHeight = karmaMetrics.actualBoundingBoxAscent + karmaMetrics.actualBoundingBoxDescent;

      const karmaBoxLeft = 1900 - karmaWidth - 48 * 2;
      const karmaBoxTop = karmaTopPlacing - headFontSize;
      const karmaBoxWidth = karmaWidth + 48 * 2;
      const karmaBoxHeight = karmaHeight + 24 * 2;
      const karmaBoxBorderRadius = karmaBoxHeight / 4;
      ...
      ctx.font = `700 ${headFontSize}px "Roboto"`;
      ctx.fillText(karmaText, 1900 - karmaWidth - 48, karmaTopPlacing);
